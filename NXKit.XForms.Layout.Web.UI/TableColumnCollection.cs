...
namespace NXKit.XForms.Layout.Web.UI
{
    ...
    public class TableColumnCollection : IEnumerable<TableColumn>
    {
        ...
        private IEnumerable<TableColumn> GetColumns()
        {
            ...
            var columnVisuals = Visual != null ? View.OpaqueChildren(Visual).OfType<TableColumnVisual>() : Enumerable.Empty<TableColumnVisual>();
        ...
        {
            foreach (var cellVisual in View.OpaqueChildren(rowVisual).OfType<TableCellVisual>())
                yield return cellVisual;
        }

        private IEnumerable<TableRowVisual> RowVisuals()
        {
            foreach (var rowVisual in View.OpaqueChildren(ColumnGroup.Table.Visual).OfType<TableRowVisual>())
            {
